...
def load_inputs(src):
    ...
    # Safe handling: if column exists, coerce to numeric and fillna; otherwise create column of zeros
    for col in ["AAY_Count", "PHH_Beneficiaries", "APL_Count"]:
        if col in fps.columns:
            fps[col] = pd.to_numeric(fps[col], errors="coerce").fillna(0.0).astype(float)
        else:
            fps[col] = 0.0
    ...
    # Respect any pre-existing Monthly_Demand_tons if present; otherwise fill from counts
    ...
    mask_use_counts = fps["Monthly_Demand_tons"] <= 0.0
