...
def run_simulation(
    ...
    # Validate FPS required columns exist (Monthly_Demand_tons may be empty but must exist)
    ...
    # Convert kg -> tons for Monthly_Demand_tons
    ...
    # Ensure stock_levels (FPS rows) have expected columns
    if 'Date' not in stock_levels.columns:
        stock_levels['Date'] = pd.NaT

    # Keep FPS rows (from earlier) and append LG rows; NOTE: FPS rows already had Date filled earlier.
