...
def run_simulation(
    ...
    # Ensure Vehicles index is preserved and Trips_Used exists (int)
    if "Trips_Used" not in vehicles.columns:
        vehicles["Trips_Used"] = 0
    else:
        vehicles["Trips_Used"] = vehicles["Trips_Used"].fillna(0).astype(int)
    # keep index stable for safe updates by index
    vehicles = vehicles.reset_index(drop=False).set_index("index")

    # Precompute daily theoretical max capacity (global guard)
    DAILY_GLOBAL_CAPACITY = TOT_V * MAX_TRIPS * TRUCK_CAP
    ...
    for day in range(1, DAYS + 1):
        ...
        # daily remaining capacity guard (prevents exceeding global capacity due to bookkeeping bugs)
        day_capacity_remaining = DAILY_GLOBAL_CAPACITY
        ...
        for _, r in fps.iterrows():
            ...
            if current <= threshold and day_capacity_remaining > 1e-9:
        ...
        # 3c) Reset vehicle usage counters for the day (preserve index)
        vehicles["Trips_Used"] = 0

        # 3d) Dispatch loop
        for urgency, fid, lgid, need_qty in needs:
            # If global daily capacity exhausted -> stop dispatching
            if day_capacity_remaining <= 1e-9:
                break
            ...
            # iterate candidate list and allocate as many trips as needed but respecting:
            # - per-trip capacity (vehicle Capacity_tons)
            # - remaining day_capacity_remaining (global guard)
            # - LG available stock and need_qty
            # choose first available candidate each time (honors preference order)
            while need_qty > 1e-9 and day_capacity_remaining > 1e-9:
                # refresh candidate set to reflect trips used
                cand = vehicles[vehicles["Mapped_LGs_List"].apply(lambda lst: lgid in lst)]
                cand = cand[cand["Trips_Used"] < MAX_TRIPS].copy()
                if cand.empty:
                    break
                ...
                chosen = cand.iloc[0]
                chosen_idx = chosen.name  # original index into vehicles
                vid = chosen["Vehicle_ID"]
                cap = float(chosen["Capacity_tons"])

                # ensure qty can't exceed per-trip cap, needed qty, lg stock, nor remaining global day capacity
                qty = min(cap, need_qty, lg_stock.get(lgid, 0.0), day_capacity_remaining)
                if qty <= 0:
                    break
                ...
                # robustly update Trips_Used by index (avoids dtype/ID matching issues)
                try:
                    vehicles.at[chosen_idx, "Trips_Used"] = int(vehicles.at[chosen_idx, "Trips_Used"]) + 1
                except Exception:
                    # fallback: try by Vehicle_ID matching
                    vehicles.loc[vehicles["Vehicle_ID"] == vid, "Trips_Used"] += 1

                # decrement need and global remaining capacity
                need_qty -= qty
                day_capacity_remaining -= qty
